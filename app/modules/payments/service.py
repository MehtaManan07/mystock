...
class PaymentsService:
    ...
    @staticmethod
    async def create(create_dto: CreateManualPaymentDto) -> Payment:
        """
        ...
        """
        def _create(db: Session) -> Payment:
    ...
    @staticmethod
    async def find_all(filters: Optional[FilterManualPaymentsDto] = None) -> List[Payment]:
        """
        ...
        """
        def _find_all(db: Session) -> List[Payment]:
            # Base query
    ...
    async def find_one(payment_id: int) -> Payment:
        ...
        """
        def _find_one(db: Session) -> Payment:
            result = db.execute(
    ...
    @staticmethod
    async def update(payment_id: int, update_dto: UpdateManualPaymentDto) -> Payment:
        """
        ...
        Raises:
            ...
            ValidationError: If validation fails
        """
        def _update(db: Session) -> Payment:
            ...
            if old_transaction_id or payment.transaction_id:
                ...
                if old_transaction_id and old_transaction_id != payment.transaction_id:
                    ...
                    result = db.execute(old_trans_query)
    ...
    @staticmethod
    async def remove(payment_id: int) -> None:
        """
    ...
    async def get_summary(
        ...
        """
        def _get_summary(db: Session) -> PaymentSummaryResponse:
        ...
        return await run_db(_get_summary)

    @staticmethod
    async def get_distinct_categories() -> dict[str, list[str]]:
        """
        ...
        """
        def _get_distinct_categories(db: Session) -> dict[str, list[str]]:
            query = select(
                Payment.type,
                Payment.category
            ).where(
        ...
        return await run_db(_get_distinct_categories)
