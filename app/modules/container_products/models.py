...
class ContainerProduct(BaseModel):
    ...
    __table_args__ = (
        Index('idx_container_product_unique', 'container_id', 'product_id', unique=True),
    )

    # Foreign Keys
    container_id: Mapped[int] = mapped_column(
        ForeignKey("container.id", ondelete="CASCADE", name="fk_container_product_container_id"),
        nullable=False
    )

    product_id: Mapped[int] = mapped_column(
        ForeignKey("product.id", ondelete="CASCADE", name="fk_container_product_product_id"),
        nullable=False
    )
